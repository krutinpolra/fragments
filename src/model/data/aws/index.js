...
// Delete a fragment's metadata and data from S3 + DynamoDB. Returns a Promise
async function deleteFragment(ownerId, id) {
  ...
  try {
    ...
    logger.info({ Bucket: s3Params.Bucket, Key: s3Params.Key }, 'Fragment data deleted from S3');
  } catch (err) {
    logger.error(
      { err, Bucket: s3Params.Bucket, Key: s3Params.Key },
  ...
  try {
    ...
    logger.info(
      { TableName: ddbParams.TableName, Key: ddbParams.Key },
      'Fragment metadata deleted from DynamoDB'
    );
  } catch (err) {
